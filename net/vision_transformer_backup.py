...
class Attention(nn.Module):
    ...
    def prepare_kv_lora(self, rank=5):
        self.kv_lora = KVLoRA(in_features=self.dim, out_features=self.dim, rank=rank)
...
class VisionTransformer(nn.Module):
    ...
    def prepare_LoRA(self, lora_layers, down_dim=5):
        self.lora_layers = lora_layers
        for idx, block in enumerate(self.blocks):
            if idx in lora_layers:
                print('block indx:{} LoRA inserted'.format(idx))
                block.attn.prepare_kv_lora(rank=down_dim)
    ...
    def forward(self, x, return_all_patches=False):
        ...
        #         embed_x = self.embedding(ori_x)
        #         etf_embed_x = self.etf_embedding(x)
        #         return embed_x, etf_embed_x
        #     else:
        #         embed_x = self.embedding(x)
        #         return embed_x
        # else:
        ...
        #         embed_x = self.embedding(ori_x)
        #         etf_embed_x = self.etf_embedding(x)
        #         return embed_x[:, 0], etf_embed_x[:, 0]
        #     else:
        #         embed_x = self.embedding(x)
        #         return embed_x[:, 0]
